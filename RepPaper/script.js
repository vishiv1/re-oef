// Representatieve JS-oplossing – groepschat met localStorage & sessionStorage
const setup = () => {
  // ===== Config =====
  const USERS = ["Jeroen", "Leila", "Alex", "Aisha", "Jageer Singh"];
  ...
  function sendMessage(rawText){
    ...
    if(!text) return;
    const msg = { text: applyEmojiShortcodes(text), sender: currentUser, timestamp: Date.now() };
    messages.unshift(msg);     // nieuw bericht logisch bovenaan
    saveMessages();
    $('#messages').prepend(renderMessage(msg)); // visueel bovenaan
  ...
  function init(){
    // vul user select
    const sel = $('#user-select'); sel.textContent = '';
    for(const u of USERS) sel.append(el('option', { value:u, textContent:u }));

    loadUser(); sel.value = currentUser;
